...
Identify keypoints and obtain joint angles

Output: 
Matrix with the keypoints
Image with the identification of the keypoints
Joint angles: elbow, knee, ankle, hand
...
if __name__ == '__main__':
    ...
    img_name_output = '/home/'+os.environ.get('USER')+'/openpose/openpose_implementations/KIMHu_image_output_openPose.png'
    datum = op.Datum()
    datum.cvInputData = dst
    opWrapper.emplaceAndPop([datum])
    newImage = datum.cvOutputData[:, :, :]
    cv2.imshow("OpenPose ", newImage)
    ...
    cv2.imwrite(img_name_output, newImage)
    cv2.destroyAllWindows()        

    for i in range(human_count):
        print('Human No. '+str(i+1))
